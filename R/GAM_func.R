GAM_eco <- function(data.model, outDir, clim.var, family="binomial", fx, k, bs, blocks){
  ...
  final.mods <- lapply(blocks,function(blockNo){
    ...
    if(!inherits(possibleError, "error")){
      ...
      return(list(Block=blockNo,AUC=AUC,mod=model1,clim.var=clim.var))
    }
  })
  # Save final model output
  save(final.mods, file=paste(outDir, "/", 
                              paste(clim.var,collapse="_"),
                              "_model_output_GAM_", family, "_Eco_block.RData",sep=""), compress="xz") 
  return(final.mods)
}

GAM_split <- function(data.model, outDir, clim.var, family="binomial", 
                      fx, k, bs,blocks){
  ...
  #return(final.mods)
  save(final.mods, file=paste(outDir, "/", climVarName,
                              "_model_output_GAM_", family, "_30_70.RData",sep=""),compress="xz")  ### modify to say where you want to save
}