GBM_eco <- function(data.model, outDir, plotPath, eval=TRUE,
                    tree.comp = c(1:3), learn.rate = c(0.01,0.001),
                    blocks, clim.var, distribution="bernoulli"){    
  ...
  write.table(perf.table,file=paste0(plotPath, "/GBM_performance.txt",sep=""))
  ...
  if(nrow(perf.table)==0) return(NULL)
    ...
    best.mod <- lapply(blocks, function(blockNo, lr, tc, nt){
      ...
      if(eval==TRUE){
        if(sum(test.dat$pres)<1) AUC <- NA
          ...
          jpeg(paste0(plotPath, "/", blockNo,"_partial_dependence", ".jpg", sep=""), height=2000, width=2000, res=300)
          list <- c(1:4)
          ...
          jpeg(paste0(plotPath, "/", blockNo,"_relative_influence", ".jpg", sep=""), height=2000, width=2000, res=300)
          par(mfrow=c(1,1), tcl=-0.5, family="serif")
          summary(mods.blockNo)
          dev.off()
          write.table(summary(mods.blockNo),file=paste0(plotPath, "/", blockNo,"_relative_influence", ".txt"))
          
        }
        return(list(block=blockNo, AUC=AUC, TSS=TSS, mod=mods.blockNo, cv.error=cv.error,learn.rate=lr,ntrees=nt,tree.complexity=tc))
      } else{
        return(list(block=blockNo, mod=mods.blockNo, cv.error=cv.error, learn.rate=lr, ntrees=nt, tree.complexity=tc))
      }
    }, lr=perf.table$lr[1], tc=perf.table$tc[1], nt=perf.table$nt[1])
    
    ## Save models
    save(best.mod, file=paste(outDir, "/", paste(clim.var,collapse="_"), 
                              "_model_output_GBM_", distribution, "_Eco_block.RData",sep=""), compress="xz")  
    return(best.mod)
    #cat('\n'); print(paste('finished  processing', species, sep=" "))
  }
}

GBM_split <- function(data.model, outDir, plotPath,
  ...
  perf.table$cv.err <- do.call('c', (lapply(mods, function(model)
    unlist(lapply(model, function(model) (min(model$cv.error)))))))
  write.table(perf.table,file=paste0(plotPath, "/GBM_performance_30_70.txt",sep=""))
  perf.table <- perf.table[perf.table$nt>1000 & perf.table$nt <= 10000,]  ## Remove any models that are based on fewer than 1000 trees as they aren't supposed to be good
  ...
  if(nrow(perf.table)==0){
    ...
    ## Starting final models
    best.mod <- lapply(1:10, function(blockNo, lr, tc, nt){
      ...
      if(eval==TRUE){
        if(sum(test.block$pres)<1){AUC <- NA; TSS <- NA} else {
          ...
          ## Save response curve plots
          jpeg(paste0(plotPath, "/",blockNo,"_partial_dependence_30_70", ".jpg", sep=""), height=2000, width=2000, res=300)
          list <- c(1:4)
          par(mfrow=c(2,2), tcl=-0.5, family="serif")
          lapply(list,function(x){plot.gbm(mods.blockNo,i.var=x,n.trees=mods.blockNo$n.trees,xlab = mods.blockNo$var.names[x])})
          dev.off()
          ...
          ## Save relative variable importance
          jpeg(paste0(plotPath, "/", blockNo,"_relative_influence_30_70", ".jpg", sep=""), height=2000, width=2000, res=300)
          par(mfrow=c(1,1), tcl=-0.5, family="serif")
          summary(mods.blockNo)
          dev.off()
          write.table(summary(mods.blockNo),file=paste0(plotPath,"/", blockNo,"_relative_influence_30_70", ".txt"))
        }
        return(list(block=blockNo, AUC=AUC, TSS=TSS, mod=mods.blockNo, cv.error=cv.error,learn.rate=lr,ntrees=nt,tree.complexity=tc))
      } else{
        return(list(block=blockNo, mod=mods.blockNo, cv.error=cv.error,learn.rate=lr,ntrees=nt,tree.complexity=tc))
    ...
    save(best.mod, file=paste(outDir, "/", climVarName,"_model_output_GBM_", distribution, "_30_70.RData",sep=""), compress="xz")  
    return(best.mod)
